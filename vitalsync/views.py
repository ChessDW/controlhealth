    ...
    context = {
        ...
        'dashboard_data': {
            ...
            'profile': {
                ...
                'isSedentary': profile.is_sedentary,
                'isOverweight': profile.is_overweight,
...
def register_user_view(request):
    ...
    if request.method == 'POST':
        ...
        medications = [
            {
                'generic_name': item.strip(),
                'commercial_name': '',
                'times_per_day': 0,
                'dosage_mg': 0,
                'affects_heart_rate': False,
                'active': True,
            }
            for item in request.POST.get('medications', '').split(',') if item.strip()
        ]
...
def chatbot_view(request):
    ...
    if not isinstance(metrics, dict):
        metrics = {}
    clean_metrics = {}
    for key in ('systolic', 'diastolic', 'heart_rate', 'anxiety_estimate', 'steps', 'sleep_hours'):
        value = metrics.get(key)
        if isinstance(value, (int, float)) and not isinstance(value, bool):
            clean_metrics[key] = value
    position = metrics.get('position')
    if isinstance(position, str) and position.strip() in ('sentado', 'parado', 'acostado'):
        clean_metrics['position'] = position.strip()
...
def profile_update_view(request):
    ...
    def clean_medications(value):
        ...
        for item in value[:20]:
            if not isinstance(item, dict):
                continue
            generic_name = str(item.get('generic_name', '')).strip()[:120]
            commercial_name = str(item.get('commercial_name', '')).strip()[:120]
            if not generic_name and not commercial_name:
                continue
            try:
                times_per_day = int(item.get('times_per_day') or 0)
            except (TypeError, ValueError):
                times_per_day = 0
            try:
                dosage_mg = float(item.get('dosage_mg') or 0)
            except (TypeError, ValueError):
                dosage_mg = 0
            cleaned.append({
                'generic_name': generic_name,
                'commercial_name': commercial_name,
                'times_per_day': max(0, times_per_day),
                'dosage_mg': max(0, dosage_mg),
                'affects_heart_rate': bool(item.get('affects_heart_rate')),
                'active': bool(item.get('active', True)),
            })
        return cleaned

    def clean_int(value, min_v, max_v):
        try:
            parsed = int(value)
        except (TypeError, ValueError):
            return None
    ...
    profile.current_medications = clean_medications(payload.get('medications'))

    sex = str(payload.get('sex', '')).strip().upper()
    profile.sex = sex if sex in dict(UserProfile.SEX_CHOICES) else ''
    profile.age = clean_int(payload.get('age'), 0, 120)
    profile.resting_heart_rate = clean_int(payload.get('resting_heart_rate'), 30, 220)
    profile.is_athlete = bool(payload.get('is_athlete'))
    profile.is_sedentary = bool(payload.get('is_sedentary'))
    profile.is_overweight = bool(payload.get('is_overweight'))
